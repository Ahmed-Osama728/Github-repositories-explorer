...
export function RepositoryCard({ repository }: RepositoryCardProps) {
  ...
  const handleStarClick = async () => {
    try {
      await toggleStar(repository.owner.login, repository.name, isStarred || false);
    ...
    <div className="p-6 bg-white rounded-lg shadow-md hover:shadow-lg transition-shadow">
      <div className="flex items-start justify-between">
        <div className="flex-1">
          ...
          <p className="mt-2 text-gray-700 line-clamp-2" title={repository.description || "No description available"}>
        ...
        <button
          ...
          className={clsx(
            ...
            isStarred ? "bg-yellow-100 text-yellow-700 hover:bg-yellow-200" : "bg-gray-100 text-gray-700 hover:bg-gray-200"
          )}
          aria-label={isStarred ? "Unstar repository" : "Star repository"}
        >
          <Star className={clsx("w-4 h-4", isStarred ? "fill-yellow-500" : "")} />
